...
def fetch_server_configs(url):
    ...
    try:
        response = requests.get(url, timeout=REQUEST_TIMEOUT) # Используем основной таймаут для скачивания списка
...
def main():
    ...
    for i, server_details in enumerate(proxies_to_process):
        ...
        try:
            ...
            # Вызываем обновленную функцию test_server_connection, которая делает обе проверки
    ...
    if good_proxies:
        output_data = {"proxies": good_proxies}
        try:
        ...
        print("К сожалению, работающих серверов, не возвращающих 403 для Google AI Studio (и прошедших предварительную проверку), не найдено среди протестированных.")
